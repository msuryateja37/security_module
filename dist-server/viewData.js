...
async function checkCloudData() {
    ...
    try {
        // 1. Incidents
        const incidents = await query('SELECT id, refNo, province, status, lossValue FROM incidents');
        ...
        // 2. Checklist Items
        const checklists = await query('SELECT id, category, task, completed FROM checklist_items');
        ...
        const statsCount = await query('SELECT COUNT(*) as count FROM performance_stats');
