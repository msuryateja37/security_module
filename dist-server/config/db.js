...
export async function getDbConnection() {
    if (useMssql) {
        ...
        const config = process.env.AZURE_SQL_CONNECTIONSTRING
...
async function initializeMssql(pool) {
...
async function seedDatabaseSqlite(connection) {
    console.log('Seeding SQLite mock records...');
    const checklists = getChecklistSeed();
    for (const item of checklists) {
        await connection.run(`INSERT INTO checklist_items (id, category, task, completed, notes) VALUES (?, ?, ?, ?, ?)`, [item.id, item.category, item.task, item.completed, item.notes]);
    }
    const incidents = getIncidentSeed();
    for (const incident of incidents) {
        await connection.run(`INSERT INTO incidents (
    ...
    for (const stat of stats) {
        await connection.run(`INSERT INTO performance_stats (province, indicator, monthlyValues) VALUES (?, ?, ?)`, [stat.province, stat.indicator, stat.monthlyValues]);
    }
    // Seed BTO reports
    const btos = getBtoSeed();
    for (const bto of btos) {
        await connection.run(`INSERT INTO bto_reports (
    ...
    for (const inv of invs) {
        await connection.run(`INSERT INTO investigation_reports (
    ...
    for (const qtr of qtrs) {
        await connection.run(`INSERT INTO quarterly_reports (
    ...
    for (const tra of tras) {
        await connection.run(`INSERT INTO tra_audits (
...
async function seedDatabaseMssql(pool) {
